...
Modified by @willyg302 to allow embedding in a custom REPL
'''
import sys
import re
import textwrap
...
def parse_defaults(doc):
	...
	for s in parse_section('options:', doc):
		# @TODO corner case "bla: options: --foo"
...
def formal_usage(section):
	...
	pu = section.split()
	return '( {} )'.format(' '.join(') | (' if s == pu[0] else s for s in pu[1:]))
...
def docopt(doc, argv=None, help=True, version=None, options_first=False):
	...
	* Full documentation is available in README.rst as well as online
	...
	try:  # @willyg302 this is so stupid but it works so hah
		raise DocoptExit()
	except DocoptExit, e:
		return e
	return None
